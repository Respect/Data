...
class MixedTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $children_1 = Mixed::with('foo', 'bar')->bar();
        $children_2 = Mixed::with('bat', 'bar')->baz()->bat();
        ...
        $this->assertEquals(array('foo', 'bar'), $children_1->getExtra('mixins'));
        $this->assertEquals(array('bat', 'bar'), $children_2->getExtra('mixins'));
    }
